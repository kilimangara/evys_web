    ...
    return (
      <div key={theme.id} onClick={bind(this.onThemeClickInfo, this, theme)}>
        <HoverPaper style={{height:200}}>
          <GridTile title={`${theme.num}. ${theme.name}`}
                    subtitle={<b>{hidden}</b>}
                    actionIcon={<IconButton onClick={this.onThemeClick.bind(this, theme.id)}
                                iconStyle={{color:grey200}} iconClassName='far fa-play-circle'
                                tooltip={'Вопросы'} tooltipPosition={'top-center'}/>}>
    ...
    return(
      <div style={styles.container}>
        ...
        <Modal ref={ref => this.modalUpdate = ref}>
          <ThemeCreation onThemeDelete={this.onThemeDelete} updateMode
...
export default connect(mapStateToProps, {loadThemesBySubject, createThemeBySubject, deleteTheme, updateTheme})(withGetScreen(ThemesScreen))
