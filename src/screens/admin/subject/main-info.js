...
export default class MainInfo extends React.Component {
  ...
  fieldChanged = (field) => (event) => {
    ...
    this.props.subjectUpdated(produce(this.props.subject, (draft) => {draft[field] = value}))
  }
  ...
  render(){
    ...
    return(
      ...
      <div style={{display: 'flex', justifyContent: 'flex-start',flexWrap: 'wrap'}}>
       {subject.tags && subject.tags.map((t, index) => <TagChip key={index} variant='outlined' label={`#${t}`} onDelete={this.deleteTag(index)}/>)}
      </div>
      <div>
