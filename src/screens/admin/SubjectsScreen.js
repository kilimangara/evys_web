    ...
    return (
      <div key={subject.id} onClick={this.onClickSubjectInfo.bind(this, subject)}>
        <HoverPaper style={{height:200}}>
          <GridTile title={subject.subject}
                    subtitle={<b>{subject.grade_representation}</b>}
                    actionIcon={<IconButton onClick={this.onClickSubject.bind(this, subject.id)}
                                iconStyle={{color:grey200}} iconClassName='far fa-play-circle'
                                tooltip={'Темы'} tooltipPosition={'top-center'}/>}>
              <img src={IMG_SRC}/>
          </GridTile>
        </HoverPaper>
      </div>
    ...
    return(
      <div style={styles.container}>
        ...
        <Modal ref={ref => this.modalUpdate = ref}>
          <SubjectCreation initialState={this.state.selectedSubject}
...
export default connect(mapStateToProps, {loadSubjects, createSubject, updateSubject, deleteSubject})(withGetScreen(SubjectsScreen))
