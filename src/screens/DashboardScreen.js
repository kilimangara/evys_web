...
class DashboardScreen extends Component {
    ...
    renderRepeat = (repeat, index) => {
        return (
            <Card>
                <ListItem
                    key={index}
                    primaryText={`Тема: ${repeat.test_block.theme.name}`}
    ...
    getFormatText(percent) {
        ...
        return <p style={{ fontSize: 14, color: 'black' }}>Ваш результат не изменился</p>
    }
            ...
            <div style={styles.containerStyle}>
                <div style={styles.innerContainer}>
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                        <VictoryChart responsive domainPadding={10} animate>
                            <VictoryAxis
                                scale="time"
                                style={{
                                ...
                                tickFormat={x => moment(x.getTime()).format('DD-MM(HH:mm)')}
                            />
                            <VictoryAxis
                                dependentAxis
                ...
                <div>
                    ...
                    <List>{this.props.repeats.map(this.renderRepeat)}</List>
                </div>
            </div>
        )
    }
