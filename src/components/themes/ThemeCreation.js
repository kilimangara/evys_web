...
export default class ThemeCreation extends Component {
  ...
  render(){
    const {name, num, is_hidden, tests_model_type,
       end_range, repetition_range, required_repeats} = this.state
    return (
      <div style={styles.container}>
        <TextField onChange={this.saveToState.bind(this, 'name')}
          floatingLabelText="Название" value={name}
          underlineFocusStyle={{borderColor: grey900}} />
        <TextField onChange={this.saveToState.bind(this, 'num')}
          floatingLabelText="Номер" value={num}
          type={'number'}
          underlineFocusStyle={{borderColor: grey900}} />
        <TextField onChange={this.saveToState.bind(this, 'required_repeats')}
          floatingLabelText="Минимальное число повторений" value={required_repeats}
          type={'number'}
          underlineFocusStyle={{borderColor: grey900}} />
        <SelectField
        ...
        <DurationPicker valueChanged={this.saveToStateDuration.bind(this, 'repetition_range')}
            defaultValue={this.toInternalValue(repetition_range)}
            labelText={'Время между повторениями'} />
        <DurationPicker valueChanged={this.saveToStateDuration.bind(this,'end_range')}
